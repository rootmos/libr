TEST_SUITE(base32, {
    TEST(encode_empty, {
         ...
         assert(LIBR(base32_encode)(enc, NULL, 0) == 1);
         assert(strcmp(enc, "") == 0);
    });
    ...
    TEST(encode_f, {
         ...
         assert(LIBR(base32_encode)(enc, "f", 1) == sizeof(enc));
    ...
    TEST(encode_fo, {
         ...
         assert(LIBR(base32_encode)(enc, "fo", 2) == sizeof(enc));
    ...
    TEST(encode_foo, {
         ...
         assert(LIBR(base32_encode)(enc, "foo", 3) == sizeof(enc));
    ...
    TEST(encode_foob, {
         ...
         assert(LIBR(base32_encode)(enc, "foob", 4) == sizeof(enc));
    ...
    TEST(encode_fooba, {
         ...
         assert(LIBR(base32_encode)(enc, "fooba", 5) == sizeof(enc));
    ...
    TEST(encode_foobar, {
         ...
         assert(LIBR(base32_encode)(enc, "foobar", 6) == sizeof(enc));
    ...
    TEST(encode_sentence, {
         ...
         assert(LIBR(base32_encode)(enc, "foobarbazwoopwoop", 17) == sizeof(enc));
    ...
    TEST(decode_empty, {
         char* enc = "";
         unsigned char buf[0];
         assert(LIBR(base32_decode)(buf, enc, strlen(enc)) == 0);
    });

    TEST(decode_f, {
         const char* enc = "MY=====";
         char buf[1];
         assert(LIBR(base32_decode)(buf, enc, strlen(enc)) == sizeof(buf));
    ...
    TEST(round_trip_arbitrary_byte_string, {
         for(size_t i = 0; i < 100; i++) {
             ...
             char msg[L]; LIBR(random_bytes)(msg, sizeof(msg));

             char enc[BASE32_ENCODED_LENGTH(sizeof(msg))];
             assert(LIBR(base32_encode)(enc, msg, L) == sizeof(enc));

             char dec[L];
             assert(LIBR(base32_decode)(dec, enc, sizeof(enc)) == L);
