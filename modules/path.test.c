...
TEST_SUITE(path, {
    TEST(join_one_segment, {
        printf("hello\n");
        char buf[128];
        size_t r = LIBR(path_join)(LIT(buf), "foo", NULL);
        assert(r == 3);
    ...
    TEST(join_two_segments, {
        char buf[128];
        size_t r = LIBR(path_join)(LIT(buf), "foo", "bar", NULL);
        assert(r == 7);
    ...
    TEST(join_three_segments, {
        char buf[128];
        size_t r = LIBR(path_join)(LIT(buf), "foo", "bar", "baz", NULL);
        assert(r == 11);
    ...
    TEST(join_overflow_first_segment, {
        char buf[3] = {0};
        size_t r = LIBR(path_join)(LIT(buf), "foobar", NULL);
        assert(r == 6);
        assert(buf[0] == 0);
    });
    ...
    TEST(join_overlap, {
        ...
        size_t r = LIBR(path_join)(LIT(buf), "foo", "bar", NULL);
        assert(r == 7);
        r = LIBR(path_join)(LIT(buf), buf, "baz", NULL);
        assert(r == 11);
        assert(strcmp(buf, "foo/bar/baz") == 0);
    });
})
